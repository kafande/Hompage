...
(define (->delim-matcher x)
  (if (procedure? x) x					; matcher proc
      (let ((re (cond ((string? x) (sre->irregex `(: ,x)))
		      ;;((char-set? x) (re-char-set x))
		      ((char? x) (sre->irregex `(: ,(string x))))
		      ((irregex? x) x)
		      (else (error "Illegal field-reader delimiter value" x)))))
	(lambda (s i)
	  (cond ((irregex-search re s i) =>
		 (lambda (m)
                   (values (irregex-match-start-index m 0)
...
(define default-field-matcher (->delim-matcher (irregex '(+ (~ white)))))
...
(define default-suffix-matcher (->delim-matcher (irregex '(or (+ white) eos))))
(define default-infix-matcher  (->delim-matcher (irregex '(+ white))))
